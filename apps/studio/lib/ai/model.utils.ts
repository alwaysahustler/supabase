...
type ReasoningEffortFor<ModelId extends OpenAIModelId> =
  ModelId extends keyof ModelReasoningSupport
...
// Models with requiresAdvanceModelEntitlement 'base' are available to all users; 'advance' requires the assistant.advance_model entitlement.
...
const ASSISTANT_MODELS_MAP = Object.fromEntries(
  ASSISTANT_MODELS.map((m) => [m.id, m])
) as Record<AssistantModelId, (typeof ASSISTANT_MODELS)[number]>;

export const DEFAULT_ASSISTANT_BASE_MODEL_ID =
  "gpt-5.4-nano" satisfies AssistantBaseModelId;

export const DEFAULT_ASSISTANT_ADVANCE_MODEL_ID =
  "gpt-5.3-codex" satisfies AssistantModelId;
...
export function isAssistantBaseModelId(id: string): id is AssistantBaseModelId {
  return (
    ...
    !ASSISTANT_MODELS_MAP[id as AssistantModelId]
      .requiresAdvanceModelEntitlement
  );
}
...
export function getDefaultModelForProvider(
  ...
  return Object.keys(models).find((id) => models[id as Model]?.default) as
    | Model
    | undefined;
}
