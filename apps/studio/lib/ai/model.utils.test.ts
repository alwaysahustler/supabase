...
describe("model.utils", () => {
  ...
  describe("PROVIDERS registry", () => {
    ...
    it("should have exactly one default model per provider", () => {
      ...
      providers.forEach((provider) => {
        ...
        const defaultModels = Object.entries(models).filter(
    ...
    it("should have bedrock model with promptProviderOptions", () => {
      const sonnetModel =
        PROVIDERS.bedrock.models["anthropic.claude-3-7-sonnet-20250219-v1:0"];
    ...
    it("should have openai provider with providerOptions", () => {
      ...
      expect(
        PROVIDERS.openai.providerOptions?.openai?.reasoningEffort
      ).toBeUndefined();
    });
  });
  ...
  describe("assistant model registry", () => {
    it("should have non-empty base and advance tiers", () => {
      expect(
        ASSISTANT_MODELS.filter((m) => !m.requiresAdvanceModelEntitlement)
          .length
      ).toBeGreaterThan(0);
      expect(
    ...
    it("defaults should satisfy unions", () => {
      ...
      expect(defaultAssistantModelId(false)).toBe(
        DEFAULT_ASSISTANT_BASE_MODEL_ID
      );
      expect(defaultAssistantModelId(true)).toBe(
        DEFAULT_ASSISTANT_ADVANCE_MODEL_ID
      );
    });
    ...
    it("getAssistantModelEntry returns config for known ids", () => {
      expect(getAssistantModelEntry("gpt-5.4-nano").reasoningEffort).toBe(
        "low"
      );
      expect(getAssistantModelEntry("gpt-5.3-codex").reasoningEffort).toBe(
        "low"
      );
